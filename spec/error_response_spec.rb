...
describe AppAnnie::ErrorResponse do
  ...
  describe '.raise_for' do
    let(:response) { {} }
    before { response.stub(:body).and_return "{}" }

    describe "401" do
      before { response.stub(:status).and_return 401 }
    ...
    describe "429" do
      before { response.stub(:status).and_return 429 }
    ...
    describe "503" do
      before { response.stub(:status).and_return 503 }
    ...
    describe "other" do
      before { response.stub(:status).and_return nil }
